...
use clinic\models\WebUser as User;
...
class Helper
{
     ...
     * @return array
     */
    public static function getRoutesByUser($userId, $clinicId)
    {
        if (!isset(self::$_userRoutes[(int) $clinicId][(int) $userId])) {
            $cache = Configs::cache();
            if ($cache && ($routes = $cache->get([__METHOD__, $userId, $clinicId])) !== false) {
                self::$_userRoutes[(int) $clinicId][(int) $userId] = $routes;
            } else {
                $routes = static::getDefaultRoutes();
                $manager = Configs::authManager();
                foreach ($manager->getPermissionsByUser($userId, $clinicId) as $item) {
                    if ($item->name[0] === '/') {
                ...
                self::$_userRoutes[(int) $clinicId][(int) $userId] = $routes;
                if ($cache) {
                    $cache->set([__METHOD__, $userId, $clinicId], $routes, Configs::cacheDuration(), new TagDependency([
                        'tags' => Configs::CACHE_TAG,
                    ]));
                }
            }
        }
        return self::$_userRoutes[(int) $clinicId][(int) $userId];
    }
     ...
     * @return boolean
     */
    public static function checkRoute($route, $params = [], $user = null, $clinicId = null)
    {
        ...
        if ($config->strict) {
            ...
            $routes = static::getRoutesByUser($userId, $clinicId);
            if (isset($routes[$r])) {
                return true;
            }
            while (($pos = strrpos($r, '/')) > 0) {
     ...
     * @return string
     */
    public static function filterActionColumn($buttons = [], $user = null, $clinicId = null)
    {
        if (is_array($buttons)) {
            $result = [];
            foreach ($buttons as $button) {
                if (static::checkRoute($button, [], $user, $clinicId)) {
                    $result[] = "{{$button}}";
                }
            }
            return implode(' ', $result);
        }
        return preg_replace_callback('/\\{([\w\-\/]+)\\}/', function ($matches) use ($user, $clinicId) {
            return static::checkRoute($matches[1], [], $user, $clinicId) ? "{{$matches[1]}}" : '';
        }, $buttons);
    }
