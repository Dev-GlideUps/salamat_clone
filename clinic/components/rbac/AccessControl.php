...
class AccessControl extends \yii\base\ActionFilter
{
    ...
    public function beforeAction($action)
    {
        ...
        $user = $this->getUser();
        $clinicId = $user->identity->active_clinic;
        if (empty($clinicId)) {
            $action->controller->redirect(["/user/select-clinic"]);
            return false;
        }
        if (!$user->getIsGuest() && $user->identity->getIsBlocked()) {
            $this->denyAccess($user);
            return false;
        }
        if (Helper::checkRoute('/' . $actionId, Yii::$app->getRequest()->get(), $user, $clinicId)) {
            return true;
        }
        $this->denyAccess($user);
        return false;
